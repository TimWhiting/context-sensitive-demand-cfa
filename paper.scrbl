...
Demand $m$-CFA is sound with respect to a concrete albeit demand semantics called \emph{demand evaluation}\footnote{See the technical report \cite{techreport}}, which is itself sound with respect to a standard call-by-value semantics.
...
It is important to keep in mind, however, that we do \emph{not} adopt its re-binding policy.
...
The technical report~\cite{techreport} presents Demand $\infty$-CFA and demand evaluation formally.
...
A breakdown of results per program is given in the technical report \cite{techreport}.)
