...
(define (report-simple-mcfa-hash h out)
  (for ([(key value) (in-hash h)])
      (if (is-singleton-val (cons key value))
        (pretty-print `(,(show-simple-ctx key) ,value) out)
        '())
    )
  )
...
(define (run-mcfa name kind kindstring query exp m out-time)
  ...
  (match timed-result
    ...
    [(list cpu real gc gas)
     ...
     (report-simple-mcfa-hash eval-results-hash out-time)
      ...
      (for ([m (in-range 1 2)])
        ...
        (for ([example (get-examples programs programloc)])
          (match-let ([`(example ,name ,exp) example])
            (define out-time-exhaustive (open-output-file (format "tests/m~a/exhaustive_detail_~a.sexpr" m name) #:exists 'replace))
            ; (for ([gas gases])
