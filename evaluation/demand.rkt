...
(define (bod Ce ρ)
  (match Ce
    [(cons C `(λ (,x) ,e))
     (clos (cons `(bod ,x ,C) e) (cons (take-cc `(□? ,x)) ρ))]
    [(cons C `(let ([,x ,e₀]) ,e₁))
     (clos (cons `(let-bod ,x ,e₀ ,C) e₁) ρ)]))
  ...
  (match-lambda
    ...
    [(and ρ (cons cc ρ′))
     ...
     (⊔ (if (cc-determined? cc)
            ...
            ⊥
            (refine ρ))
        (>>=clos (get-refines* ρ′) (λ (ρ′) (clos (cons cc ρ′)))))]))
