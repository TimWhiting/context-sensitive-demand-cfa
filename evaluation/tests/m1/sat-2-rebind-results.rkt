'(expression:
  (letrec*
   ((phi
     (λ (x1)
       (λ (x2)
         (λ (x3)
           (λ (x4)
             (λ (x5)
               (λ (x6)
                 (λ (x7)
                   (match
                    (match
                     x1
                     ((#f) (match x2 ((#f) (app #f)) (_ (app #t))))
                     (_ (app #t)))
                    ((#f) (app #f))
                    (_
                     (match
                      (match
                       x1
                       ((#f)
                        (match
                         (app not x2)
                         ((#f)
                          (match (app not x3) ((#f) (app #f)) (_ (app #t))))
                         (_ (app #t))))
                       (_ (app #t)))
                      ((#f) (app #f))
                      (_
                       (match
                        (match
                         x3
                         ((#f) (match x4 ((#f) (app #f)) (_ (app #t))))
                         (_ (app #t)))
                        ((#f) (app #f))
                        (_
                         (match
                          (match
                           (app not x4)
                           ((#f) (match x1 ((#f) (app #f)) (_ (app #t))))
                           (_ (app #t)))
                          ((#f) (app #f))
                          (_
                           (match
                            (match
                             (app not x2)
                             ((#f)
                              (match
                               (app not x3)
                               ((#f) (app #f))
                               (_ (app #t))))
                             (_ (app #t)))
                            ((#f) (app #f))
                            (_
                             (match
                              (match
                               x4
                               ((#f) (match x2 ((#f) (app #f)) (_ (app #t))))
                               (_ (app #t)))
                              ((#f) (app #f))
                              (_ (app #t)))))))))))))))))))))
    (try
     (λ (f)
       (match
        (app f (app #t))
        ((#f) (match (app f (app #f)) ((#f) (app #f)) (_ (app #t))))
        (_ (app #t)))))
    (sat-solve-7
     (λ (p)
       (app
        try
        (λ (n1)
          (app
           try
           (λ (n2)
             (app
              try
              (λ (n3)
                (app
                 try
                 (λ (n4)
                   (app
                    try
                    (λ (n5)
                      (app
                       try
                       (λ (n6)
                         (app
                          try
                          (λ (n7)
                            (app
                             (app
                              (app (app (app (app (app p n1) n2) n3) n4) n5)
                              n6)
                             n7))))))))))))))))))
   (app sat-solve-7 phi)))

'(query:
  (app (-> (app (app (app (...) ...) n3) n4) <-) n5)
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((λ (x4) (-> (λ (x5) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n3) n4) <-) n5))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app (app (app (...) ...) n3) n4) <-) n5)
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((λ (x4) (-> (λ (x5) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n3) n4) <-) n5))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app (app (app (...) ...) n4) n5) <-) n6)
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((λ (x5) (-> (λ (x6) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n4) n5) <-) n6))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app (app (app (...) ...) n4) n5) <-) n6)
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((λ (x5) (-> (λ (x6) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n4) n5) <-) n6))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app (app (app (...) ...) n5) n6) <-) n7)
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((λ (x6) (-> (λ (x7) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n5) n6) <-) n7))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app (app (app (...) ...) n5) n6) <-) n7)
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((λ (x6) (-> (λ (x7) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n5) n6) <-) n7))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app (app (app p ...) n2) n3) <-) n4)
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((λ (x3) (-> (λ (x4) ...) <-))
  (env ((app (-> (app (app (app p ...) n2) n3) <-) n4))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app (app (app p ...) n2) n3) <-) n4)
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((λ (x3) (-> (λ (x4) ...) <-))
  (env ((app (-> (app (app (app p ...) n2) n3) <-) n4))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app (app p n1) n2) <-) n3)
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((λ (x2) (-> (λ (x3) ...) <-)) (env ((app (-> (app (app p n1) n2) <-) n3))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app (app p n1) n2) <-) n3)
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((λ (x2) (-> (λ (x3) ...) <-)) (env ((app (-> (app (app p n1) n2) <-) n3))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app p n1) <-) n2)
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((λ (x1) (-> (λ (x2) ...) <-)) (env ((app (-> (app p n1) <-) n2))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> (app p n1) <-) n2)
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((λ (x1) (-> (λ (x2) ...) <-)) (env ((app (-> (app p n1) <-) n2))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #f))
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n2) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n2) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #f))
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n3) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n3) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #f))
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n4) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n4) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #f))
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n5) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n5) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #f))
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n6) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n6) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #f))
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n7) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n7) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #f))
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n1) ...) <-))
  (env ((letrec* (phi ... sat-solve-7) (-> (app sat-solve-7 phi) <-)))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #t))
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n2) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n2) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #t))
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n3) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n3) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #t))
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n4) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n4) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #t))
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n5) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n5) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #t))
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n6) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n6) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #t))
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n7) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n7) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (-> f <-) (app #t))
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n1) ...) <-))
  (env ((letrec* (phi ... sat-solve-7) (-> (app sat-solve-7 phi) <-)))))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app (app (app (...) ...) n3) n4) (-> n5 <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app (app (app (...) ...) n3) n4) (-> n5 <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app (app (app (...) ...) n4) n5) (-> n6 <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app (app (app (...) ...) n4) n5) (-> n6 <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app (app (app (...) ...) n5) n6) (-> n7 <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app (app (app (...) ...) n5) n6) (-> n7 <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app (app (app p ...) n2) n3) (-> n4 <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app (app (app p ...) n2) n3) (-> n4 <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app (app p n1) n2) (-> n3 <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app (app p n1) n2) (-> n3 <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app p n1) (-> n2 <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app (app p n1) (-> n2 <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #f) <-))
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #f) <-))
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #f) <-))
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #f) <-))
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #f) <-))
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #f) <-))
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #f) <-))
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #t) <-))
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #t) <-))
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #t) <-))
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #t) <-))
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #t) <-))
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #t) <-))
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app f (-> (app #t) <-))
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app not (-> x2 <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app not (-> x2 <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app not (-> x3 <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app not (-> x3 <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (app not (-> x4 <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (letrec* (phi ... sat-solve-7) (-> (app sat-solve-7 phi) <-))
  (env ()))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match
   (match (app not x4) ...)
   (#f)
   (_ (-> (match (match (app not ...) ...) ...) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #f)) <-) (#f) _)
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #f)) <-) (#f) _)
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #f)) <-) (#f) _)
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #f)) <-) (#f) _)
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #f)) <-) (#f) _)
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #f)) <-) (#f) _)
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #f)) <-) (#f) _)
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #t)) <-) (#f) _)
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #t)) <-) (#f) _)
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #t)) <-) (#f) _)
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #t)) <-) (#f) _)
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #t)) <-) (#f) _)
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #t)) <-) (#f) _)
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app f (app #t)) <-) (#f) _)
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app not x2) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app not x2) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app not x3) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app not x3) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (app not x4) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (match (app not x2) ...) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (match (app not x4) ...) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (match x1 ...) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (match x1 ...) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (match x3 ...) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> (match x4 ...) <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> x1 <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> x1 <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> x1 <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> x2 <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> x2 <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> x3 <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> x4 <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (-> x4 <-) (#f) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #f)) ((#f) (-> (app #f) <-)) _)
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) (#f) (_ (-> (app #t) <-)))
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) ((#f) (-> (match (app f (app #f ...)) ...) <-)) _)
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) ((#f) (-> (match (app f (app #f ...)) ...) <-)) _)
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) ((#f) (-> (match (app f (app #f ...)) ...) <-)) _)
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) ((#f) (-> (match (app f (app #f ...)) ...) <-)) _)
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) ((#f) (-> (match (app f (app #f ...)) ...) <-)) _)
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) ((#f) (-> (match (app f (app #f ...)) ...) <-)) _)
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app f (app #t)) ((#f) (-> (match (app f (app #f ...)) ...) <-)) _)
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app not x2) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app not x2) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app not x2) ((#f) (-> (match (app not x3) ...) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app not x2) ((#f) (-> (match (app not x3) ...) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app not x3) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app not x3) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app not x3) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app not x3) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app not x4) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (app not x4) ((#f) (-> (match x1 ...) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match (app not x2) ...) (#f) (_ (-> (match (match x4 ...) ...) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match (app not x2) ...) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match (app not x4) ...) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match x1 ...) (#f) (_ (-> (match (match x1 ...) ...) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match x1 ...) (#f) (_ (-> (match (match x3 ...) ...) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match x1 ...) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match x1 ...) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match x3 ...) (#f) (_ (-> (match (match (app not ...) ...) ...) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match x3 ...) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match x4 ...) (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match (match x4 ...) ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x1 (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x1 (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x1 (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x1 ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x1 ((#f) (-> (match (app not x2) ...) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x1 ((#f) (-> (match x2 ...) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x2 (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x2 (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x2 ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x2 ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x3 (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x3 ((#f) (-> (match x4 ...) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x4 (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x4 (#f) (_ (-> (app #t) <-)))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x4 ((#f) (-> (app #f) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (match x4 ((#f) (-> (match x2 ...) <-)) _)
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n1) (-> (app try (λ (n2) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n1) (-> (app try (λ (n2) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n2) (-> (app try (λ (n3) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n2) (-> (app try (λ (n3) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n3) (-> (app try (λ (n4) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n3) (-> (app try (λ (n4) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n4) (-> (app try (λ (n5) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n4) (-> (app try (λ (n5) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n5) (-> (app try (λ (n6) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n5) (-> (app try (λ (n6) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n6) (-> (app try (λ (n7) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n6) (-> (app try (λ (n7) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n7) (-> (app (app (app (...) ...) n6) n7) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (n7) (-> (app (app (app (...) ...) n6) n7) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (p) (-> (app try (λ (n1) ...)) <-))
  (env ((letrec* (phi ... sat-solve-7) (-> (app sat-solve-7 phi) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query:
  (λ (x7) (-> (match (match x1 ...) ...) <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query: ((top) letrec* (phi ... sat-solve-7) ...) (env ()))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query: (app (-> p <-) n1) (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((letrec* (... () (phi (-> (λ (x1) ...) <-)) try ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query: (app (-> p <-) n1) (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((letrec* (... () (phi (-> (λ (x1) ...) <-)) try ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query: (app p (-> n1 <-)) (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(query: (app p (-> n1 <-)) (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  f
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n1) (-> (app try (λ (n2) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n2) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n2) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(store:
  f
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n2) (-> (app try (λ (n3) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n3) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n3) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(store:
  f
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n3) (-> (app try (λ (n4) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n4) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n4) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(store:
  f
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n4) (-> (app try (λ (n5) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n5) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n5) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(store:
  f
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n5) (-> (app try (λ (n6) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n6) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n6) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(store:
  f
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (n6) (-> (app try (λ (n7) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n7) ...) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
	'((app try (-> (λ (n7) ...) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
literals: '(⊥ ⊥ ⊥)

'(store:
  f
  (λ (f) (-> (match (app f (app #t ...)) ...) <-))
  (env ((λ (p) (-> (app try (λ (n1) ...)) <-)))))
clos/con:
	'((app try (-> (λ (n1) ...) <-))
  (env ((letrec* (phi ... sat-solve-7) (-> (app sat-solve-7 phi) <-)))))
literals: '(⊥ ⊥ ⊥)

'(store:
  n1
  (λ (n1) (-> (app try (λ (n2) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n1
  (λ (n1) (-> (app try (λ (n2) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n2
  (λ (n2) (-> (app try (λ (n3) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n2
  (λ (n2) (-> (app try (λ (n3) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n3
  (λ (n3) (-> (app try (λ (n4) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n3
  (λ (n3) (-> (app try (λ (n4) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n4
  (λ (n4) (-> (app try (λ (n5) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n4
  (λ (n4) (-> (app try (λ (n5) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n5
  (λ (n5) (-> (app try (λ (n6) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n5
  (λ (n5) (-> (app try (λ (n6) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n6
  (λ (n6) (-> (app try (λ (n7) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n6
  (λ (n6) (-> (app try (λ (n7) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n7
  (λ (n7) (-> (app (app (app (...) ...) n6) n7) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((con #f) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  n7
  (λ (n7) (-> (app (app (app (...) ...) n6) n7) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  p
  (λ (p) (-> (app try (λ (n1) ...)) <-))
  (env ((letrec* (phi ... sat-solve-7) (-> (app sat-solve-7 phi) <-)))))
clos/con:
	'((letrec* (... () (phi (-> (λ (x1) ...) <-)) try ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  p
  (λ (p) (-> (app try (λ (n1) ...)) <-))
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((letrec* (... () (phi (-> (λ (x1) ...) <-)) try ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  p
  (λ (p) (-> (app try (λ (n1) ...)) <-))
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((letrec* (... () (phi (-> (λ (x1) ...) <-)) try ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  phi
  (letrec* (... () (phi (-> (λ (x1) ...) <-)) try ...) ...)
  (env ()))
clos/con:
	'((letrec* (... () (phi (-> (λ (x1) ...) <-)) try ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  sat-solve-7
  (letrec* (... try (sat-solve-7 (-> (λ (p) ...) <-)) () ...) ...)
  (env ()))
clos/con:
	'((letrec* (... try (sat-solve-7 (-> (λ (p) ...) <-)) () ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  try
  (letrec* (... phi (try (-> (λ (f) ...) <-)) sat-solve-7 ...) ...)
  (env ((letrec* (phi ... sat-solve-7) (-> (app sat-solve-7 phi) <-)))))
clos/con:
	'((letrec* (... phi (try (-> (λ (f) ...) <-)) sat-solve-7 ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  try
  (letrec* (... phi (try (-> (λ (f) ...) <-)) sat-solve-7 ...) ...)
  (env ((match (-> (app f (app #f)) <-) (#f) _))))
clos/con:
	'((letrec* (... phi (try (-> (λ (f) ...) <-)) sat-solve-7 ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  try
  (letrec* (... phi (try (-> (λ (f) ...) <-)) sat-solve-7 ...) ...)
  (env ((match (-> (app f (app #t)) <-) (#f) _))))
clos/con:
	'((letrec* (... phi (try (-> (λ (f) ...) <-)) sat-solve-7 ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  try
  (letrec* (... phi (try (-> (λ (f) ...) <-)) sat-solve-7 ...) ...)
  (env ()))
clos/con:
	'((letrec* (... phi (try (-> (λ (f) ...) <-)) sat-solve-7 ...) ...) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x1
  (λ (x1) (-> (λ (x2) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n3) n4) <-) n5))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x1
  (λ (x1) (-> (λ (x2) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n4) n5) <-) n6))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x1
  (λ (x1) (-> (λ (x2) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n5) n6) <-) n7))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x1
  (λ (x1) (-> (λ (x2) ...) <-))
  (env ((app (-> (app (app (app p ...) n2) n3) <-) n4))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x1
  (λ (x1) (-> (λ (x2) ...) <-))
  (env ((app (-> (app (app p n1) n2) <-) n3))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x1
  (λ (x1) (-> (λ (x2) ...) <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x2
  (λ (x2) (-> (λ (x3) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n3) n4) <-) n5))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x2
  (λ (x2) (-> (λ (x3) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n4) n5) <-) n6))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x2
  (λ (x2) (-> (λ (x3) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n5) n6) <-) n7))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x2
  (λ (x2) (-> (λ (x3) ...) <-))
  (env ((app (-> (app (app (app p ...) n2) n3) <-) n4))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x2
  (λ (x2) (-> (λ (x3) ...) <-))
  (env ((app (-> (app (app p n1) n2) <-) n3))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x2
  (λ (x2) (-> (λ (x3) ...) <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x3
  (λ (x3) (-> (λ (x4) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n3) n4) <-) n5))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x3
  (λ (x3) (-> (λ (x4) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n4) n5) <-) n6))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x3
  (λ (x3) (-> (λ (x4) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n5) n6) <-) n7))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x3
  (λ (x3) (-> (λ (x4) ...) <-))
  (env ((app (-> (app (app (app p ...) n2) n3) <-) n4))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x3
  (λ (x3) (-> (λ (x4) ...) <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x4
  (λ (x4) (-> (λ (x5) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n3) n4) <-) n5))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x4
  (λ (x4) (-> (λ (x5) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n4) n5) <-) n6))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x4
  (λ (x4) (-> (λ (x5) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n5) n6) <-) n7))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x4
  (λ (x4) (-> (λ (x5) ...) <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x5
  (λ (x5) (-> (λ (x6) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n4) n5) <-) n6))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x6
  (λ (x6) (-> (λ (x7) ...) <-))
  (env ((app (-> (app (app (app (...) ...) n5) n6) <-) n7))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store:
  x7
  (λ (x7) (-> (match (match x1 ...) ...) <-))
  (env ((λ (n7) (-> (app (app (app (...) ...) n6) n7) <-)))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)

'(store: x1 (λ (x1) (-> (λ (x2) ...) <-)) (env ((app (-> (app p n1) <-) n2))))
clos/con:
	'((con #f) (env ()))
	'((con #t) (env ()))
literals: '(⊥ ⊥ ⊥)
