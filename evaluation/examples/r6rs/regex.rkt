...
(define (regex-derivative re c)
  ...
  (cond
    ...
    ((match-alt re     (lambda (pat1 pat2)
                         (alt (regex-derivative pat1 c) (regex-derivative pat2 c)))))
...
;(check-expect (regex-derivative 'baz 'f)
;              #f)

;(check-expect (regex-derivative '(seq foo barn) 'foo)
;  'barn)
...
;(check-expect (regex-derivative '(alt (seq foo bar) (seq foo (rep baz))) 'foo)
;              '(alt bar (rep baz)))
