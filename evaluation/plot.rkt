  ...
  (let ([results (list)])
    (for ([m (range 5)])
      (for ([program all-programs])
        (for ([t timeouts])
          (set! results (append (read-file (format "tests/m~a/~a-gas_~a.sexpr" m program t)) results))
          )
        )
      )
    (for ([m (range 5)])
      (for ([program all-programs])
        (set! results (append (read-file (format "tests/m~a/exhaustive_~a-gas~a.sexpr" m program)) results))
        )
      )
    results
    )
