...
(module+ main
  ...
  (for ([m (in-range 0 (+ 1 max-context-length))])
    (let ([basic-cost 0]
          ...
          [basic-acc-cost 0])
      (current-m m)
      (for ([example (get-examples '(ack blur cpstak eta sat-small))]) ; ack blur cpstak deriv eta facehugger flatten kcfa-2 kcfa-3 loop2-1 map mj09 primtest regex rsa sat-1 sat-2 sat-3 tak))])
