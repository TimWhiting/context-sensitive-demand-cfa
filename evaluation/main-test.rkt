...
(module+ main
  ...
  (for ([m (in-range 0 (+ 1 max-context-length))])
    (let ([basic-cost 0]
          ...
          [basic-acc-cost 0])
      (current-m m)
      (for ([example (get-examples '(ack blur cpstak eta facehugger  kcfa-2 kcfa-3 loop2-1 sat-1 sat-2 tak sat-small))]) ;  deriv flatten map mj09 primtest regex sat-3 rsa))])
